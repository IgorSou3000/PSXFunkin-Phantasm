...
static struct
{
	...
	u8 page, next_page;
	boolean page_swap;
	u8 select, next_select, skip_selectdown, skip_selectup;
	...
	//Menu assets
	Gfx_Tex tex_back, tex_ng, tex_story, tex_title, tex_crediti;
...
//Stage HUD functions
static void Stage_DrawHealthMenu(s16 x, s16 y, u8 i)
{
	if (i == 3)
		x += 80;
...
void Menu_Load(MenuPage page)
{
	...
	Gfx_LoadTex(&stage.tex_hud1, IO_Read("\\STAGE\\HUD1MENU.TIM;1"), GFX_LOADTEX_FREE);
	
	menu.sonicm = Char_SonicM_New(FIXED_DEC(25,1), FIXED_DEC(40,1));
	...
	//Fallthrough
		...
		case MenuPage_Story:
		{
			static const struct
			{
				...
				{NULL, StageId_1_1, StageId_1_1Multi, "INNER DEMONS", {"PHANTASM", NULL, NULL}},
			};
			...
			Menu_DifficultySelector(SCREEN_WIDTH - 75, 180);
			...
			if (menu.next_page == menu.page && Trans_Idle())
			{
				...
				if (pad_state.press & (PAD_START | PAD_CROSS))
				{
					...
					menu.page_param.stage.id = menu_options[menu.select].stagemulti;
			...
			//Draw Sonicm
			menu.sonicm->tick(menu.sonicm);
			
			//Draw upper strip
			RECT name_bar = {0, 32, SCREEN_WIDTH, 135};
			...
			if (menu.next_page == menu.page || menu.next_page == MenuPage_Main)
			{
				...
				for (u8 i = 0; i < COUNT_OF(menu_options); i++)
				{
					...
					if (y >= SCREEN_HEIGHT)
						break;
					Menu_DrawWeek(menu_options[i].week, 90, y + 100);
				}
			}
			else if (animf_count & 2)
			{
				//Draw selected option
				Menu_DrawWeek(menu_options[menu.select].week, 90, 64 + 100 + (menu.select * 48) - (menu.scroll >> FIXED_SHIFT));
			}
		...
		case MenuPage_Freeplay:
		{
			static const struct
			{
				...
				{StageId_1_1, StageId_1_1Multi, 0xFF9271FD, "PHANTASM", 1},
				{StageId_1_2, StageId_1_2Multi, 0xFF9271FD, "PHANTASM MONIKA", 3},
			};
			...
			for (u8 i = 0; i < COUNT_OF(menu_options); i++)
			{
				...
				Stage_DrawHealthMenu(150 + (y >> 2), SCREEN_HEIGHT2 - 24 + y, menu_options[i].icon);
		...
		menu.skip_selectup = menu.select - 1;
			...
			static const struct
			{
				...
				{0xFF9271FD, "IGORSOU", "MADE ALL THE PORT",1},
				{0xFF9271FD, "",NULL,-1},
				{0xFF9271FD, "SPECIAL THANKS",NULL,-1},
				{0xFF9271FD, "UNSTOPABLE", "BIG FRIEND", 0},
				{0xFF941653, "LUCKY","MISS AND ACCURATE CODE",7 },
				{0xFF941653, "CUCKYDEV","MAKE THE PSXFUNKIN",2 },
				{0xFF941653, "LORD SCOUT","FRIEND",6},
				{0xFF941653, "JOHN PAUL","FRIEND",4},
				{0xFF941653, "PSXFUNKIN DISCORD","DISCORD",-1},
				{0xFF941653, "ZERIBEN","FRIEND",5},
				{0xFF9271FD, "",NULL,-1},
				{0xFF9271FD, "OG CREATORS",NULL,-1},
				{0xFF941653, "GARBIE PATTIE","DIRECTOR",8},
				{0xFF941653, "ITOSAIHARA","BACKGROUND AND MISC ARTIST",9},
				{0xFF941653, "JAYYTHUNDER","SPRITE SUPPORT",-1},
				{0xFF941653, "LUCKYPUPPET","CHARTER",10},
				{0xFF941653, "BIDDLE", "COMPOSER",11},
				{0xFF941653, "MURASAKI", "PROGRAMMER",-1},
				{0xFF9271FD, "",NULL,-1},
				{0xFF9271FD, "MONIKA VERSION",NULL,-1},
				{0xFF941653, "CYCOXIUS", "SPRITES AND ASSETS",-1},
				{0xFF941653, "SUPERSTAMPS", "COMPOSER", 12},
				{0xFF9271FD, "",NULL,-1},
				{0xFF9271FD, "CHARTER FOR FNF MULTIPLAYER",NULL,-1},
				{0xFF941653, "AS ADRIAN XD", "I GUESS IT OBVIOUS",-1},
			};
			...
			if (menu.next_page == menu.page && Trans_Idle())
			{
				...
				if (pad_state.press & PAD_DOWN)
				{
					if (menu_options[menu.skip_selectdown].text[2] == '\0')
					    menu.select += 2;
		...
		case MenuPage_Options:
		{
			...
			static const struct
			{
				...
				{OptType_Boolean, "PSYCH CAMERA", &stage.movecamera, {.spec_boolean = {0}}},
