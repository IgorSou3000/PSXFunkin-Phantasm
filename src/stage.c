...
static void Stage_LoadNotes(void)
{
	...
	note_x[1] = FIXED_DEC(60,1) + FIXED_DEC(SCREEN_WIDEADD,4);
	note_x[2] = FIXED_DEC(94,1) + FIXED_DEC(SCREEN_WIDEADD,4);
	note_x[3] = FIXED_DEC(128,1) + FIXED_DEC(SCREEN_WIDEADD,4);
	//Opponent
	note_x[4] = note_x[5] = note_x[6] = note_x[7] = FIXED_DEC(-408,1) - FIXED_DEC(SCREEN_WIDEADD,4);
	}
	else
	{
	//BF
	...
	note_x[5] = FIXED_DEC(-94,1) - FIXED_DEC(SCREEN_WIDEADD,4);
...
{
	//Get src and dst
	fixed_t hx = (128 << FIXED_SHIFT) * (10000 - health) / 12000;
	RECT src = {
		(i % 5) * 64,
		16 + (i / 5) * 64,
		64,
	...
	RECT_FIXED dst = {
		hx + ox * FIXED_DEC(12,1) - FIXED_DEC(37,1),
		FIXED_DEC(SCREEN_HEIGHT2 - 48 + 4 - 12, 1),
		src.w << FIXED_SHIFT,
		src.h << FIXED_SHIFT
	};
	if (stage.downscroll)
...
static void Stage_DrawHealthBar(u8 x, s32 color)
{
	u8 barr = (color >> 16) & 0xFF;
	u8 barg = (color >> 8) & 0xFF;
	u8 barb = (color) & 0xFF;

	//Draw health bar
	RECT health_fill = {0, 0, x, 4};
	RECT_FIXED health_dst = {FIXED_DEC(-128,1), (SCREEN_HEIGHT2 - 32) << FIXED_SHIFT, 0, FIXED_DEC(4,1)};
	if (stage.downscroll)
		health_dst.y = -health_dst.y - health_dst.h;

	 //draw healthbar
	health_dst.w = health_fill.w << FIXED_SHIFT;
	Stage_DrawTexCol(&stage.tex_hud1, &health_fill, &health_dst, stage.bump, barr >> 1, barg >> 1, barb >> 1);
}
...
void Stage_Load(StageId id, StageDiff difficulty, boolean story)
{
	...
	//Initialize camera
	if (stage.cur_section->flag & SECTION_FLAG_OPPFOCUS || stage.mode == StageMode_2P)
	...
	//health bar color if opponent be NULL
	if (stage.stage_def->ochar.new == NULL)
	stage.player_state[1].character->health_b = stage.player_state[0].character->health_b;
...
void Stage_Tick(void)
{
	...
	switch (stage.state)
	{
		...
		if (stage.mode != StageMode_2P)
		{
			...
			//hardcode swap to fleetway sonic/lunatic monika
			if (stage.mode != StageMode_2P)
			{
			...
			if (stage.mode < StageMode_2P)
			{
				...
				Stage_DrawHealth(stage.player_state[0].health, (stage.opponent == NULL) ? 9 : stage.opponent->health_i, -1);

				//Draw health bar
				Stage_DrawHealthBar(245 - (245 * stage.player_state[0].health / 20000), stage.player_state[1].character->health_b);
				Stage_DrawHealthBar(245, stage.player_state[0].character->health_b);	
			}
		...
		if (hit_type == 0)
		{
			for (int i = 0; i < 3; i++)
			{
				...
				Obj_Splash *splash = Obj_Splash_New(
					...
					note_y[(note->type & 0x7) ^ stage.note_swap] * (stage.downscroll ? -1 : 1),
				     type
				);
				if (splash != NULL)
					ObjectList_Add(&stage.objlist_splash, (Object*)splash);
